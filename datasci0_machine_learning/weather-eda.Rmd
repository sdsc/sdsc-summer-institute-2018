...
author: "SDSC Summer Institute 2018, Mai H. Nguyen"
modified: "2018-07-24"
date: "`r Sys.time()`"
...
<<<FILL-IN>>>(df$Rainfall) / length(df$Rainfall)      # Average daily rainfall
```
...
<<<FILL-IN>>> <- c("Rainfall","Evaporation","Sunshine","WindSpeed9am","Humidity9am","Pressure9am","Cloud9am","Temp9am")
...
```{r barplot}
table(df$WindGustDir)
plot(df$WindGustDir,col="lightblue",main="Distribution of Wind Gust Direction",xlab=<<<FILL-IN>>>,ylab="Count")
```

### Scatter plot of Evaporation vs. Temperature
```{r scatterplot}
cor(df$Evaporation,df$Temp9am)
plot(df$Evaporation,df$Temp9am,col="blue",main="Evaporation vs. Temperature",xlab="Evaporation (mm)",ylab=<<<FILL-IN>>>)
```
...
<<<FILL-IN>>>(lm(df$Temp9am ~ df$Evaporation), col="red")
```
...
ggplot(df, aes(x=as.factor(month(Date)), y=MaxTemp)) + <<<FILL-IN>>>(fill="lightblue") + 
