...
```{r  removeRiskMM}
df$RISK_MM <- NULL
<<<FILL-IN>>>(df)           # Get dimensions of dataframe
```
...
<<<FILL-IN>>>(df.tst)
<<<FILL-IN>>>(df.tst$RainTomorrow)/nrow(df.tst)
# summary(df.tst)
...
saveRDS(<<<FILL-IN>>>, "weather-tst.rds")
```
...
library(rpart)
<<<FILL-IN>>>             # Load rpart.plot library
...
<<<FILL-IN>>>(df.trn)               # Columns in datasets after removing some variables
...
paste("Error on TEST Data (Pruned Tree): ", <<<FILL-IN>>>)
...
# df.tst.imp <- df.tst
sum(<<<FILL-IN>>>(df.tst.imp))                         # Number of NAs after imputation
